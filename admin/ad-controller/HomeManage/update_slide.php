<?php
    ...
    $slide_list = get_slide_by_id($_GET['id']);
    $slide = $slide_list[0];
    if(isset($_POST['btn-updateslide'])){
        ...
        update_slide_by_id($_GET['id'], $name, $imageName, $hide, $ordered);
        add_dieuchinhslide($_GET['id'], $_SESSION['admin']['adid'], "Sửa");
        $_SESSION['alert'] = [
...
<div class="form-container">
    <form action="ad-controller/HomeManage/update_slide.php?id=<?php echo $_GET['id']; ?>" method="post" enctype="multipart/form-data">
        <!-- Thêm trường ẩn để lưu ID ảnh -->
        <input type="hidden" name="slide_id" value="<?php echo $slide['id']; ?>">
        
        <div class="form-group">
            <label for="name">Tên ảnh:</label>
            <input type="text" id="name" name="name" maxlength="255" value="<?php echo htmlspecialchars($slide['name']); ?>" required>
        </div>
        
        <!-- Thêm trường ẩn để lưu tên ảnh hiện tại -->
        <input type="hidden" name="current_image" value="<?php echo htmlspecialchars($slide['image']); ?>">
        
        <div class="form-group">
            <label for="image">Ảnh mới(nếu có):</label>
            <input type="file" id="image" name="image" accept="image/*" onchange="previewImage(this, 'preview1')">
            <img id="preview1" src="<?php echo !empty($slide['image']) ? '../upload/image/slide/'.htmlspecialchars($slide['image']) : '#'; ?>" 
                 alt="Preview image" class="file-preview" 
                 style="display: <?php echo !empty($slide['image']) ? 'block' : 'none'; ?>;">
        </div>
        
        
        <div class="form-group checkbox-container">
            <input type="checkbox" id="hide" name="hide" value="1" <?php echo ($slide['hide'] == 1) ? 'checked' : ''; ?>>
            <label for="hide">Ẩn</label>
        </div>
        
        <div class="form-group">
            <label for="ordered">Thứ tự hiển thị:</label>
            <input type="number" id="ordered" name="ordered" min="0" value="<?php echo (int)$slide['ordered']; ?>">
        </div>
        
        <div class="btn-container">
            <button type="submit" class="btn" name="btn-updateslide">Cập nhật thông tin ảnh</button>
